...
/**
 * CardMatchingHard 클래스는 난이도 상의 카드 매칭 게임을 나타냅니다.
 * JFrame을 확장하며 게임의 그래픽 사용자 인터페이스(GUI)를 포함합니다.
 */
public class CardMatchingHard extends JPanel {
    ...
    public Level3Timer level3timer;
    /** 게임에 사용되는 카드 목록입니다. */
     ...
     */
    public CardMatchingHard(CardLayout layout, JPanel panel, MainPage mainPage, String[] data) {
        cardLayout = layout;
        this.panel = panel;
        ...
        //타이머 추가
        level3timer = new Level3Timer(score);
        JProgressBar timerVisible = level3timer.getProgressBar();
        topPanel.add(timerVisible);
        level3timer.addfinishPanel(cardLayout, panel, mainPage, score.getScore());
        ...
        int cardWidth = (int) (parentFrame.getWidth()* 0.06);
        int cardHeight = (int) (parentFrame.getHeight()* 0.12);
                ...
                Timer cardTimer = new Timer(40, new ActionListener() {
                    ...
                    public void actionPerformed(ActionEvent e) {
                        if (selectedCard.icon.equals(clickedCard.icon)) {
                            ...
                            pairsFound++;
                            if (pairsFound == 24) {
                                int timerScore = level3timer.getTimerValue();
